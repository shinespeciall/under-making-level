...
#define Sub_8017ADC_GmWarioChng_Fire ((void (*)(unsigned char)) 0x8017ADD)
...
int Sub_806D4C0_SetWarioAnimation_MapFieldAtari(unsigned short Wario_y, unsigned short Wario_x)
{
    ...
    if (cGmStartFlg == 1 && cPauseFlag == 0 && soft_reset == 0 && !usWarStopFlg)
    {
        unsigned short eventId_Layer1;
        eventId_Layer1 = CurrentTileset_EventIdTablePointer[Layer1_DecompressedDataPointer[tile_id]];
        if (eventId_Layer1 == 0x37 && (ADDR_KEY_4 & 1)) // press A
        {
            if (!Wario_ucReact)
            {
                if (Wario_ucStat <= 4) // walk, turn, stop, jump
                {
                    Sub_8012BAC_GmWarioChng(4);; goto reset_Yspeed;
                }
                if (Wario_ucStat > 4 && Wario_ucStat <= 9) // squat, sslip, swalk, sjump, sturn
                {
                    Sub_8012BAC_GmWarioChng(8); goto reset_Yspeed;
                }
                if (Wario_ucStat > 12 && Wario_ucStat <= 16) // rollB, roll, rjump, rflip
                {
                    Sub_8012BAC_GmWarioChng(15); goto reset_Yspeed;
                }
                if (Wario_ucStat = 22 || Wario_ucStat == 24) // attack, d-attack
                {
                    if (Wario_sMvSpeedX_lowbyte == 0x80)
                    {
                        Sub_8012BAC_GmWarioChng(25);
                    }
                    else
                    {
                        Sub_8012BAC_GmWarioChng(24);
                    }
                    goto reset_Yspeed;
                }
                if (Wario_ucStat = 23 || Wario_ucStat == 25) // d-attack
                {
                    Sub_8012BAC_GmWarioChng(25); goto reset_Yspeed;
                }
            }
            if (Wario_ucReact == 2)
            {
                if (Wario_ucStat <= 2)
                {
                    Sub_8017ADC_GmWarioChng_Fire(2); goto reset_Yspeed;
                }
                if (Wario_ucStat <= 4)
                {
                    Sub_8017ADC_GmWarioChng_Fire(4); goto reset_Yspeed;
                }
                if (Wario_ucStat <= 6)
                {
                    Sub_8017ADC_GmWarioChng_Fire(6); goto reset_Yspeed;
                }
                if (Wario_ucStat <= 8)
                {
                    Sub_8017ADC_GmWarioChng_Fire(8); goto reset_Yspeed;
                }
            }
reset_Yspeed:
            Wario_sMvSpeedY = 300;
    ...
    if ( !v11.anum ) // if nothing in Layer 1
    {
        ...
        else
        {
            if (L2_tile_terrain_type == 12) // vanilla upwards one-way
            {
                ...
                if(cGmStartFlg == 1 && cPauseFlag == 0 && soft_reset == 0 && (ADDR_KEY_4 & 0x80))
                {
