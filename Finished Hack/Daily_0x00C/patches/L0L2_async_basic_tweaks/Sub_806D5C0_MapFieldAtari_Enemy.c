...
int Sub_806D5C0_MapFieldAttact_Enemy(unsigned short Enemy_y, unsigned short Enemy_x)
{
    ...
    if ( !l_Mhit.anum ) // if nothing in Layer 1
    {
        ...
        if ( L2_tile_terrain_type <= 12 && ((CurrentRoomHeader_Layer2MappingType & 0x10) == 0x10) )
        {
            tmp_result = Sub_8094E00_call_via_r1(&(l_Mhit.anum), Tile_collision_fucntionTable[L2_tile_terrain_type]);
        }
        else
        {
            if ( (l_Mhit.anum - 38) > 7 || ((CurrentRoomHeader_Layer2MappingType & 0x10) != 0x10) )
            {
                return result;
            }
