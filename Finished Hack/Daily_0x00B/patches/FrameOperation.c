...
void FrameOperations()
{
    ...
    {
        // Room 0, vertical Room insta warp
        if (CurrentRoom == 0 || CurrentRoom == 1 || CurrentRoom == 5 || CurrentRoom == 7)
        {
            if (WarioYPos <= 0x0040) {
                WarioYPos = 0x036F;
            } else if (WarioYPos >= 0x0370) {
                WarioYPos = 0x0041;
            }
            for (int c = 0 ; c < 24 ; c ++) {
                if (ENTITIES[c].CurEnemy_YPos <= 0x0040) {
                    ENTITIES[c].CurEnemy_YPos = 0x036F;
                } else if (ENTITIES[c].CurEnemy_YPos >= 0x0370) {
                    ENTITIES[c].CurEnemy_YPos = 0x0041;
                }
            }
        }
    }

    // Fix Wario gets stuck when taking damage
    if (Wario_ucReact == 0 && Wario_ucStat == 0x13 && myflag_Iscounting == 0)
    {
        WarioInvincibility = 0x90;
        Wario_ucTimer = 0;
        myflag_Iscounting = 1;
        Wario_ucStat = 0;
    }
    ...
    // Fix Wario gets stuck when bounce away by enemy, or after B dash
    if (Wario_ucReact == 0 && (Wario_ucStat == 26 || Wario_ucStat == 21))
    {
        Wario_ucStat = 0;
    }
