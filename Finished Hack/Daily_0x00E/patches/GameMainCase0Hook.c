...
#define Sub_806BB4C_GmapBgVramInitSet ((void (*)(unsigned char)) 0x806BB4D)
...
void GameMainCase0Hook()
{
    ...
    // custom logic, set this in any case
    unk_3000039 = 0;
    ...
    // always reload the layer 1 for Room 6 when reload room or use the door and enter the room
    if (!CurrentPassage && !CurrentLevel && !from_unpause)
    {
        // Room 6 is loaded
        if (CurrentRoom == 6)
        {
            Sub_806BDF8_MapLH_kaitou_RLEDecompress(1, *CurrentRoomHeader_Layer1CompressedDataPointer, Layer1_DecompressedDataPointer);
            Sub_806BB4C_GmapBgVramInitSet(1); // refresh layer 1 vram
        }
    }
}
